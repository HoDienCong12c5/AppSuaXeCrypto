...
const page = ( p ) => {
  ...
  const {token, balance, history, user} = p.props;
  let base64Logo = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAOEAA..';
  const toAdd = '0x5270b3da7df9b03ba997d065575bb77ffdd3f39a';
  const renderItem =( {item, index} ) =>{
    console.log( item );
    const isUser = item.sdtSend === user.sdt;
    return(
      <History key={index} item={item} isUser={isUser} />
    )
  }
  return (
        ...
        <View style={styles.containerPlatform}>
          <View style={{flex:7}}>
            ...
            <View style={styles.containerSupport}>
              <Text style={styles.amount}>{In18.web3.amount}: {balance} {!token ?'TOMO' :' ETH'} </Text>
            </View>
          </View>
